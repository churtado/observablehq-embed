// URL: https://observablehq.com/@keesterbrugge/adgoji-dashboard-visualizations
// Title: adgoji dashboard visualizations
// Author: keesterbrugge (@keesterbrugge)
// Version: 447
// Runtime version: 1

const m0 = {
    id: "e7ec89937d6f4afe@447",
    variables: [
      {
        inputs: ["md"],
        value: (function(md){return(
  md`# adgoji dashboard visualizations
  
  TODO:
  - make apps selectable in the bar chart (third plot) and reflect the changes in the first plot.
  - add confidence interval around prediction
  - Repair the budget view. The group by aggregation broke it, since all apps don't have the same budget values.`
  )})
      },
      {
        name: "frequency",
  ...
  0.05
      ...
      },
      {
        name: "calc_uniques",
        value: (function(){return(
  function(c,max_uniques,cpm,f){
    return Math.max(0,Math.min(Math.log(Math.sin(f*cpm*100) + 100*cpm*f) * c, max_uniques) )
  }
  )})
      },
      {
        ...
        inputs: ["calc_uniques"],
        value: (function(calc_uniques){return(
  function(c, max_uniques, cpm, f, app){
    let uniques = calc_uniques(c, max_uniques, cpm, f)
    return {"cpm": cpm, "uniques": uniques,  "budget" : cpm*uniques, "app": app}
  }
  )})
      },
      {
        name: "data2",
        inputs: ["max_cpm","make_observation","frequency"],
        value: (function(max_cpm,make_observation,frequency)
  {
    let out = []
    for (let cpm = .01; cpm  <= max_cpm ; cpm += 0.01){
      out = out.concat([
        make_observation(0.8, 3, cpm, frequency, "A"), 
        make_observation(0.6, 2.5, cpm, frequency, "B"), 
        make_observation(0.4, 1.6, cpm, frequency, "C"), 
        ])
    }
    return out
  }
      ...
      {
        inputs: ["vl","data2","select_with"],
        value: (function(vl,data2,select_with){return(
  vl({  
     "data":{  
        "values":data2
     },
     "vconcat":[  
        {  
           ...
           "encoding":{  
              ...
              "y":{  
                 ...
                 "type":"quantitative",
                 "axis":{  
                    "title":"uniques in thousands, millions, whatever"
                 }
              },
      ...
      {
        name: "viewof select_with",
        inputs: ["DOM"],
        value: (function(DOM){return(
  DOM.select(["cpm","budget"])
  )})
      },
      {
        name: "select_with",
        inputs: ["Generators","viewof select_with"],
        value: (G, _) => G.input(_)
      },
      {
        inputs: ["md"],
        value: (function(md){return(
  md`# ^^ only look up. Everything below is test stuff.`
  )})
      },
      {
        name: "max_cpm",
  ...
  10
      ...
      {
        inputs: ["frequency","max_cpm"],
        value: (function(frequency,max_cpm){return(
  `freq: ${frequency}, max_cpm: ${max_cpm}`
  )})
      },
      {
        name: "data",
        inputs: ["max_cpm","frequency"],
        value: (function(max_cpm,frequency)
  {
    let out = []
    for (let i = 1; i  <= max_cpm*100 ; i++){
      let cpm = i/100;
      let uniques = Math.min(Math.log(Math.sin(frequency*i) + i*frequency) * 0.8, 3) ; // nondecreasing
      let elem = {"cpm": cpm, "uniques": uniques,  "budget" : cpm*uniques};
      out.push(elem)
    }
    return out
  }
  ...
  JSON.stringify(data2)
      ...
      },
      {
        inputs: ["md"],
        value: (function(md){return(
  md`drag in second plot to choose x-axis domain`
  )})
      },
      {
  ...
  //  ],
    ...
    "mark": "line",
    "encoding": { 
      "x" : {"field": select_with, "type" : "quantitative",
            "scale": {"domain": {"selection": "brush"}} 
            },
      "y" : {"field": "uniques", "type" : "quantitative",
            "axis": {"title": "uniques in thousands, millions, whatever"}},
    ...
    },
    {
      ...
      "encoding": {
        ...
        "mark": "bar",
          "encoding": {
      "x": {"field": "app", "type": "nominal"},
      "y": {"aggregate": "max", "field": "uniques", "type": "quantitative"}
        }
       }]
  })
  )})
  ...
  vl({
    ...
    "encoding": { 
      ...
      },
      {
        inputs: ["md"],
        value: (function(md){return(
  md`# volume vs cpm visualization & parallel coordinate
  
  have os selector
  have predicted mean volume
  have confidence intervals
  have slider for cpm
  show realised data
  show some areas better prediction than others
  
  ignore seasonality, ignore # observed uniques
  ignore the fact that we choose cpm instead of observing it in nature (selection bias)
  lets assume that we use 1 bidprice per campaign per day. 
  
  data observation 4 vars: day, app, cpm, n_uniques.
  you can't infer from an observation what you're result would've been that day with other cpm. Not as bound either. Lower cpm could've resulted in winning the idfa later for lower price, therefor not upper bound. Lower cpm could've resulted in not being able to bid, because there is other campiagn that bids higher, therfor not lower bound. 
  
  Try glm plot for relation bertween volume and cpm
  Also try out parallel coordinate plot. 
  `
  )})
      },
      {
  ...
  require("https://d3js.org/d3.v5.min.js")
      ...
      },
      {
        name: "mtcars",
        inputs: ["d35"],
        value: (function(d35){return(
  d35.csvParse(`name,economy (mpg),cylinders,displacement (cc),power (hp),weight (lb),0-60 mph (s),year
  ...
  `)
      ...
      },
      {
        name: "chart",
        inputs: ["d33","mtcars"],
        value: (function(d33,mtcars)
  {
  var margin = {top: 30, right: 10, bottom: 10, left: 10},
      width = 960 - margin.left - margin.right,
      height = 500 - margin.top - margin.bottom;
  
  var x = d33.scale.ordinal().rangePoints([0, width], 1),
      y = {};
  
  var line = d33.svg.line(),
      axis = d33.svg.axis().orient("left"),
      background,
      foreground;
  
  var svg = d33.select("body").append("svg")
      .attr("width", width + margin.left + margin.right)
      .attr("height", height + margin.top + margin.bottom)
    .append("g")
      .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
  var dimensions = {}
  //d33.csv("cars.csv", function(error, cars) {
   var cars = mtcars
    // Extract the list of dimensions and create a scale for each.
    x.domain( dimensions = d33.keys(cars[0]).filter(function(d) {
      return d != "name" && (y[d] = d33.scale.linear()
          .domain(d33.extent(cars, function(p) { return +p[d]; }))
          .range([height, 0]));
    }));
    ...
    g.append("g")
        .attr("class", "axis")
        .each(function(d) { d33.select(this).call(axis.scale(y[d])); })
      .append("text")
    ...
    g.append("g")
        .attr("class", "brush")
        .each(function(d) { d33.select(this).call(y[d].brush = d33.svg.brush().y(y[d]).on("brush", brush)); })
      .selectAll("rect")
        .attr("x", -8)
        .attr("width", 16);
  //});
  ...
  function brush() {
    ...
    return svg.node();
      ...
      },
      {
        inputs: ["md"],
        value: (function(md){return(
  md`## parallel coordinate plot
  
  sources:
  - https://github.com/mlarosa2/parcoords: multibrush
  - https://github.com/BigFatDog/parcoords-es: no multibrush`
  )})
      },
      {
        name: "vl",
        inputs: ["require"],
        value: (function(require){return(
  require("@observablehq/vega-lite@0.0")
  )})
      },
      {
        name: "vega",
        inputs: ["require"],
        value: (function(require){return(
  require("@observablehq/vega")
  )})
      },
      {
        inputs: ["vega","spec"],
        value: (function(vega,spec){return(
  vega(spec)
  )})
      },
      {
